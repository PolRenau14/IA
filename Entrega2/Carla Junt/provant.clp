...
(defrule determina_preu_saltat_o_no ""
  ...
  (if (or (> ?viatgeact1 ?maxpress)(> ?viatgeact2 ?maxpress))
    then 
      (if (eq ?pres_pres no)
        then 
          (assert (condicions no_es_poden_complir1))
      )
  )
)
...
(defrule assigna_llocs_interes2 ""
  ...
  (presupost ?press)
  =>
  (printout t "HEY, QUE ENTRO AQUI TAMBÉ" crlf)
...
(defrule peatges "Determina si vols passar per peatges amb cotxe"
  ...
  (not (peatge-cotxe))
  =>
  (bind ?answer (yes-or-no-p "Vol pagar peatges durant el seu trajecte amb cotxe? (yes/no)"))
...
(defrule prescindir "Determina si prescindir si sobrepassa el pressupost"
  ...
  =>
  (if(yes-or-no-p "En cas que es sobrepassi el seu pressupost però es compleixin totes les altres condicions pel viatge, acceptarà el preu assignat? (yes/no)")
...
(defrule cotxe-autonomia "Determina autonomia"
  ...
  =>
  (if (yes-or-no-p "Vols que el cotxe sigui llogat? (yes/no)")
...
(defrule transport-pel-lloc "determina com moure's per el lloc"
  (not (transport-local ?))
  =>
  (bind ?question "Indiqui amb quin tipus de transport es vol mobilitzar per la ciutat: autobus/metro/tren/cotxe/altre")
  (bind ?answer (ask-question ?question autobus metro tren cotxe altre))
...
(defrule determina-presupost "Preguntem sobre el presupost"
  (not(presupost ?))
  =>
  (bind ?question "Quin es el presupost per persona per a tot el viatge? (baix(200-1000€) / mitja(1001-2000€) / alt(+3000€))")
...
(defrule system-banner ""
 ;;; (declare (salience 100))
  ...
  (printout t "Sistema de personalització de viatges per a majors d'edat.")
  (printout t crlf crlf))
...
(defrule resultado-consejo2 "Regla salida de consejo"
 ...
 (nom ?name)
  =>
  (printout t "---------------------------------" crlf)
  (printout t "Benvolgut/da " ?name "," crlf crlf "No s'ha trobat cap viatge amb les condicions que ha indicat, haurà de canviar alguna cosa. " crlf)
  (halt)
)
